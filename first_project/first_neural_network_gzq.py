...
class NeuralNetwork(object):
    ...
    #if features[1,3] and targets[1,1]
    def train(self, features, targets):
        ...
        for X, y in zip(features, targets):
            ...
            #hidden_input(1,2) hidden_output(1,2)
            hidden_inputs = np.dot(X,self.weights_input_to_hidden) # signals into hidden layer
            hidden_outputs = self.sigmoid(hidden_inputs) # signals from hidden layer

            # TODO: O|utput layer - Replace these values with your calculations.
            #final_input(1,1) final_outputs(1,1)
            ...
            #error(1,1)  output_error_term(1,1)
            ...
            #keep Wih's shape equal (3,2)
            delta_weights_i_h += hidden_error_term * X[:,None]
            # Weight step (hidden to output)
            #keep Who's shape equal (2,1)
...
# #### 请将你的答案填写在下方
# 
# 1.前半个月的预测效果较好，后半个月与实际数据相差较大
# 2.整体预测效果不好，对后期对指导意义不大
# 3.数据量不够大，对预测结果影响较大
# 4.为了满足整体趋势，学习率较大，收敛效果不好，同时，12月份后半月对单车使用数量历史参考依据，估算相对不准确
# 5.12月后半月出现了使用数量锐减，可能缺少了分析年对维度
# 
